...
import { getStoredUserProfile, type StoredUserProfile } from '@/lib/nutrilens';
...
export const Analytics = () => {
  ...
  const calculateBMI = () => {
    if (!userData?.heightCm || !userData?.weightKg) return { bmi: 0, status: 'Unknown', color: 'gray' };
  ...
  const currentWeight = userData?.weightKg ?? 71;
  const goalWeight = userData?.targetWeight ?? 76;
