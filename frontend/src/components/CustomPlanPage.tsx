...
import { syncUserProfileFromStorage } from '@/lib/nutrilens';
...
export const CustomPlanPage: React.FC<CustomPlanPageProps> = ({
  ...
  useEffect(() => {
    ...
    localStorage.setItem('goalTargetWeight', plan.targetWeight.toString());
    localStorage.setItem('goalTargetDate', plan.targetDate);
  ...
  const handleGetStarted = async () => {
    try {
      ...
      toast({
        title: 'Unable to set up your profile',
    ...
    <div className="min-h-screen bg-background flex flex-col p-6">
      ...
      <div className="mt-auto">
        <Button
          ...
          size="lg"
        >
          {isSubmitting ? 'Setting up your profile...' : 'Let\'s get started!'}
        </Button>
      </div>
    </div>
  );
